...
export const useSubgraph = (stream: number) => {
	const FRIENDS_QUERY = `
	{
		userStreams(where: {stream: "${stream}"}) {
			id
			user {
			id
				}
...
export const useFriendInfo = (stream: number, address: string | undefined) => {
	const FRIEND_QUERY = `
    {
		userStreams(where: {id: "${stream + " " + address?.toLowerCase()}"}) {
			id
			user {
				id
				}
